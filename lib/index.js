...
module.exports = function (app) {
    app.on(['pull_request.opened', 'pull_request.reopened', 'pull_request.labeled', 'pull_request.edited', 'pull_request_review'], function (context) { return __awaiter(void 0, void 0, void 0, function () {
        var config, pr, prLabels, labelsToAdd, prHasAppliedLabels, approvedReviewDismissed, reviewParams, reviews, autoapprovalReviews, reviewDismissed, prUser, ownerSatisfied, missingRequiredLabels, blacklistedLabels, prParams, labels;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    app.log(context);
                    return [4 /*yield*/, getConfig(context, 'autoapproval.yml')];
                case 1:
                    config = _a.sent();
                    context.log(config, 'Loaded config');
                    pr = context.payload.pull_request;
                    prLabels = pr.labels.map(function (label) { return label.name; });
                    labelsToAdd = config.apply_labels;
                    prHasAppliedLabels = labelsToAdd.length > 0 && labelsToAdd.every(function (label) { return prLabels.includes(label); });
                    approvedReviewDismissed = false;
                    if (!context.payload.review) return [3 /*break*/, 3];
                ...
                case 2:
                    ...
                    reviewDismissed = context.payload.action === 'dismissed';
                ...
                case 3:
                    ...
                    ownerSatisfied = config.from_owner.length === 0 || config.from_owner.includes(prUser);
                    ...
                    if (!(missingRequiredLabels.length === 0 && ownerSatisfied && blacklistedLabels.length === 0)) return [3 /*break*/, 7];
                ...
                case 4:
                    ...
                    labels = context.issue({ labels: labelsToAdd });
                    return [4 /*yield*/, context.github.issues.addLabels(labels)];
                case 5:
                    _a.sent();
                    _a.label = 6;
